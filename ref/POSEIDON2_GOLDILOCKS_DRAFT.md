...
- 代码接口已按本规范演进；
- 已替换为真实的 Goldilocks 域运算 + sponge + 置换流程；
- 轮常量已改为编译期静态常量表（不再运行时生成）；
- `poseidon2_hash_bytes()` 已改为流式 absorb（不再进行 VLA 拼接）；
- 在替换为正式参数表并完成外部向量比对前，暂不宣称“标准参数完全一致”。
...
- 当前工程实现采用 `RF=8, RP=22` 与 `x^7` S-box。若要与某一官方/生态参数集完全一致，应以该参数集的固定轮常量与线性层矩阵替换当前工程参数。

## 3.1 轮常量策略（当前实现）

当前 `poseidon2.c` 的轮常量采用编译期固定表，目标是：

- 确保实现可复现、可测试、可回归；
- 在未最终敲定外部参数表前，先把接口与流程稳定下来。

注意：

- 该策略用于工程推进，不应替代最终选定的标准参数表。
- 最终版本应将轮常量来源（上游参数表/脚本）一并固定并可审计。
