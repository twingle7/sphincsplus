...
#include "poseidon2.h"
#include "poseidon2_params_goldilocks_profile.h"
...
/* Internal mix driven by fixed diagonal profile constants. */
static void p2_mix_internal(uint64_t state[SPX_POSEIDON2_T])
{
    ...
    for (i = 0; i < SPX_POSEIDON2_T; i++) {
        state[i] = p2_add(sum, p2_mul(P2_INTERNAL_DIAG_12[i], state[i]));
    }
}
                          ...
                          const uint8_t *input, size_t inlen)
{
    ...
    poseidon2_inc_absorb(&ctx, domain, domainlen);
    poseidon2_inc_absorb(&ctx, input, inlen);
    poseidon2_inc_finalize(&ctx);
    poseidon2_inc_squeeze(output, outlen, &ctx);
}
