...
static void fill_air_metadata(spx_p2_stark_stats *out_stats)
{
                                   ...
                                   const spx_p2_ffi_private_witness *wit)
{
    ...
    t0 = monotonic_ms();
    ret = spx_p2_ffi_generate_pi_f(&blob, pub, wit);
    t1 = monotonic_ms();
    ...
    t0 = monotonic_ms();
    ret = spx_p2_ffi_verify_pi_f(&blob, pub);
    t1 = monotonic_ms();
    if (ret != SPX_P2_FFI_STATUS_OK)
    {
        free(rows);
                               ...
                               size_t public_ctx_len)
{
    ...
    if (spx_p2_stark_collect_ffi_stats(out_stats, &pub, &wit) != 0)
    {
        return -1;
    }
